...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li> --->*correct*
...
```{r samp1-dist}
samp1 %>%
  summarise(samp_med = median(area), 
            samp_iqr = IQR(area),
...
<li> Sample size of 1000. </li> --->*correct*
</ol>
...
ggplot(price_sample_small, aes(x = x_bar)) +
  geom_histogram(bins = 30)
...
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li> -->*correct*
