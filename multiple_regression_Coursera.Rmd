...
<ol>
<li> Observational study </li> -->*correct*
<li> Experiment </li>
</ol>
...
<li> Yes, revise wording to "Is there an association between beauty and course evaluations?" </li> -->*correct*
...
<li> The left skewness of the data suggests that the students are less likely to rate the professors highly. </li> -->*correct*
</ol>
```{r score-dist}
summary(evals$score)
hist(evals$score)
...
```{r two-vars-eda}
ggplot(evals, aes(x = bty_avg, y = cls_students)) +
  geom_point()
...
<li> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </li> -->*correct*
...
# Check nearly normal distribution via histogram
ggplot(m_bty_gen, aes(x = .resid)) +
  geom_histogram(bins = 30) +
  xlab('Residuals')
# or a normal probability plot of residuals
qqnorm(m_bty_gen$resid)
qqline(m_bty_gen$resid)
...
<li> Tenure Track, Tenured, Teaching </li> -->*correct*
...
```{r m_full, tidy = FALSE, warning = FALSE}
...
<li> 0.12 points higher than minority professors, all else held constant. </li> -->*correct*
