...
# * We could use count(sfo_feb_flights) to get the same amount but `nrow` function is usually used*
...
<ol>
<li> The distribution is unimodal. </li> --->*correct*
...
ggplot(sfo_feb_flights, aes(x = arr_time)) +
...
```{r on-time-arr-perc}
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay < 0, "on time", "delay"))
